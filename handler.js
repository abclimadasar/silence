        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                try {
                    ...
                    if (m.limit)
                        m.reply('Kamu menggunakan fitur limit\n⌬  -1 Limit')
                }
                break
            }
        }
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        conn.sendButtonImg(id, await (await fetch(action === 'add' ? wel : lea)).buffer(), text, author, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', fkontak, {
                            contextInfo: {
                                externalAdReply: {
...
global.dfail = (type, m, conn) => {
    ...
    if (msg) return conn.reply(m.chat, msg, false, {
        quoted: m,
        contextInfo: {
            externalAdReply: {
                showAdAttribution: true,
                ...
                title: wm,
                body: titlebot,
                sourceUrl: 'https://chat.whatsapp.com/L5jhSrREsbjCV2es6mSTG6'
            }
        }
    })
    ...
    if (msgg) return conn.sendButton(m.chat, `━━━━━━▢ Verify ▢━━━━━━`, msgg, null, ['- Verify -', '.verify'], m)
}
