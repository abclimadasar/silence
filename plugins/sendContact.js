let handler = async(m, { conn }) => {
  ...
  await conn.sendContactArray(m.chat, [
    [`${owner[0]}`, `AngelDark`, `👑 Developer BOT`, `🚫 Don't call Developer`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://linktr.ee/whatsappbotz`, `Just Try!!🪐`],
    [`${sok2.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't Spam/Call me 🙂`, `Nothing`, `🏴‍☠️ Asia!`, `🚀 https://linktr.ee/whatsappbotz`, `Hanya bot biasa yang kadang suka eror!!`],
    ...
    [`${sok4.split('@')[0]}`, `Smileyヅ`, `🎗 Team Developer BOT`, `🚫 Don't call Owner`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://linktr.ee/whatsappbotz`, `Just Do It!!👾`],
  ], fkontak) 
}
handler.help = ['ownerr']
handler.tags = ['info']
